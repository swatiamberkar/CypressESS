...
describe('Causal Leave', function() {
	...
	let DayType =
		[{LeaveType: "CL", FromDate: "01/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "01/04/"+FinancialYear_From, ToDateDay: "FIRST HALF DAY"},
		 {LeaveType: "CasualLeave", FromDate: "01/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "01/04/"+FinancialYear_From, ToDateDay: "SECOND HALF DAY"}, 
		 {LeaveType: "Casual Leave", FromDate: "01/04"+FinancialYear_From, FromDateDay: "FIRST HALF DAY", ToDate: "01/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},
	...
	let LeaveDate =
		[{LeaveType: "CL", FromDate: "02/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "01/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},
		 {LeaveType: "CL", FromDate: "12/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "12/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},//before Current Day
		 {LeaveType: "CL", FromDate: "02/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "02/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},//Current Day 
		 {LeaveType: "CL", FromDate: "26/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "26/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},//After Current day
		 {LeaveType: "CL", FromDate: "15/04/"+FinancialYear_From, FromDateDay: "FULL DAY", ToDate: "16/04/"+FinancialYear_From, ToDateDay: "FULL DAY"},
		];	
	...
	it('Apply Causal Leave of Past Date',function(){
	const { softAssert, softExpect } = chai;
	...
	it('Apply Casual Leave of Future date',function(){
		const { softAssert, softExpect } = chai;
		cy.get('#drpLeaveType').select(DayType[0].LeaveType,{force: true})
		cy.wait(5000)
		cy.get('#txtFromDate').click().then(input => {
	...
	it('Apply Casual Leave for two days',function(){
		const { softAssert, softExpect } = chai;
		cy.get('#drpLeaveType').select(DayType[0].LeaveType,{force: true})
		cy.wait(5000)
		cy.get('#txtFromDate').click().then(input => {
				input[0].dispatchEvent(new Event('input', { bubbles: true }))
				input.val(LeaveDate[4].FromDate)
			})
		cy.get('#drpFromLeaveTyp').select(LeaveDate[4].FromDateDay,{force: true})
		
		cy.get('#txtToDate').click().then(input => {
				input[0].dispatchEvent(new Event('input', { bubbles: true }))
				input.val(LeaveDate[4].ToDate)
			})
		cy.get('#drpToLeaveTyp').select(LeaveDate[4].ToDateDay,{force: true})
		
		
		cy.get('#btnAdd').click({force: true})
		 
			cy.get('[data-title="From Date"]').invoke('text').then((text) => {
		 softExpect(text.trim()).to.eq(LeaveDate[4].FromDate);
		})
		
		cy.get('[data-title="Half/Full Day"]').eq(0).invoke('text').then((text) => {
		 softExpect(text.trim()).to.eq(LeaveDate[4].ToDateDay);
		})
		
		cy.get('[data-title="To Date"]').invoke('text').then((text) => {
		 softExpect(text.trim()).to.eq(LeaveDate[4].ToDate);
		})
	...
	it('Verify Notification Leave at Employee', function() {
		const { softAssert, softExpect } = chai;
		cy.EmployeeLogin()
		cy.get('[onclick="clearNotification()"]').eq(0).click({force: true})
		cy.wait(2000)
