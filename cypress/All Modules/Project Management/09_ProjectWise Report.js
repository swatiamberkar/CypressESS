...
describe('09_ProjectWise Report', function () {
	...
	it('Navigate to My Report', function() {	
		cy.visit(Cypress.env('url') +'ProjectManagement/Transaction/ProjectManagementMyReport');
		cy.get('#ProjectWiseReportNew').click();
			cy.wait(3000)
	})

	it('Download Report', function () {
		cy.get('#ddMonth').select(Month);
		cy.xpath("//span[contains(text(),'Choose')]").click({ force: true })
