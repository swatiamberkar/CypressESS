...
describe('Permission', function() {
	...
	it('Assign manager for Permission module', function() {	
		...
		// Success Validation	
			...
			expect(text.trim()).to.contain('Permission')
	...
	it('Login into Pocket ESS & Navigate to Permission', function() {	
		cy.EmployeeLogin()
		cy.visit(Cypress.env('url')+'Leave/Transaction/LeaveRequest?Menu=permission')

	})
	
	it('Verify Validation Massges - Please add data first!', function() {
		const { softAssert, softExpect } = chai;
		
		 cy.get('#btnSave').click({force: true})
	
		 cy.get(".noty_body").invoke('text').then((text) => {
		 softExpect(text.trim()).to.eq('Please add data first!');
	...
	it('Verify Validation Massges - Shift Schedule Not Found.!', function() {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('url')+'Leave/Transaction/LeaveRequest?Menu=permission')
	...
	it('Verify Validation Massges - Data saved successfully For Approval!', function() {
		...
		cy.get('#tmPermiHour').click({force: true})
		cy.get('#tmPermiHour').clear().type(PermiHour);
		
		cy.get('#tmPermiMin').click({force: true})
		cy.get('#tmPermiMin').clear().type(tmPermiMin);
		...
		cy.get('#txtReason').clear().type('Test');
		 ...
		 cy.get('#tblPermi >tbody >tr:nth-child(2)>td:nth-child(2)').contains(PermiHour +':'+tmPermiMin)
	...
	it('Verify Validation Massges - Duplicate Permission Date not Allowed.', function() {
		const { softAssert, softExpect } = chai;
	...
	it('Verify Permission in Permission Approval pending', function() {
		...
		cy.get('.xcontainer > div  > table > tbody >tr>td:nth-child(2)').eq(0).contains(PermissionDate)
		cy.get('.xcontainer > div  > table > tbody >tr>td:nth-child(2)').eq(1).contains(PermiHour +':'+tmPermiMin)
		cy.get('.xcontainer > div  > table > tbody >tr>td:nth-child(2)').eq(2).contains(Reason)
	})
